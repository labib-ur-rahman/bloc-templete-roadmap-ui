# Design system — bloc_template_roadmap_ui
...
- **Seed color:** `#ff6a1a` (used to derive `ColorScheme` via `ColorScheme.fromSeed` where applicable).
...
Primary family: **HindSiliguri**

- **HindSiliguri** — 5 file(s) configured in `pubspec.yaml` (app-wide `fontFamily` when primary)
...
Import tokens via `package:bloc_template_roadmap_ui/src/theme/theme_constants.dart`.
